...
def minmax_newton_low(drawable, epsilon = 0.001):
	low = 0
	high = 255
	...
	while True:
		...
		if count == 16: break
		pt = int((low + high) / 2)
		_, _, _, _, _, _, percentile = drawable.histogram(Gimp.HistogramChannel.VALUE, pt / 255.0, 1.0)
		if percentile >= threshold:
			...
			new_low = low
			new_high = pt
		# print(f"{low}-{high} {pt} ({percentile}) {new_low}-{new_high}")
		if low == new_low and high == new_high: break
		low = new_low
		high = new_high
	return low - 1

def minmax_newton_high(drawable, start_low, epsilon = 0.001):
	low_norm = start_low / 255.0
	low = start_low
	high = 255
	threshold = 1.0 - epsilon
	_, _, _, _, _, _, volume = drawable.histogram(Gimp.HistogramChannel.VALUE, low_norm, 1.0)
	...
	while True:
		...
		if count == 16: break
		pt = int((low + high) / 2)
		_, _, _, _, _, _, percentile = drawable.histogram(Gimp.HistogramChannel.VALUE, low_norm, high / 255.0)
		if percentile > threshold:
			new_low = low
			new_high = pt
...
def minmax(drawable, epsilon = 0.001):
	low = minmax_newton_low(drawable, epsilon)
	high = minmax_newton_high(drawable, low, epsilon)
	return low, high
...
sand_layer = img.get_layer_by_name('sand')
new_low_v, new_high_v = minmax(sand_layer)

# signature
signature = "afiril"
font_size = 72
text_vertical_offset = 100
font = Gimp.Font.get_by_name('Sans-serif Bold Italic')
hex_color = format(indent_v, 'x')
Gimp.context_set_foreground(Gegl.Color.new(f"#{hex_color}{hex_color}{hex_color}ff"))
