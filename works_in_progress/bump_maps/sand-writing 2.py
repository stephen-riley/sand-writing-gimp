...
def set_text_background_color(image, sand, text, ratio=0.8):
...
set_text_background_color(img, sand_layer, text_layer)

text_layer.invert(False)

spread_layers = [spread(copy_layer(img, text_layer, f"text - {dist}"), dist) for dist in [10, 15, 20, 25, 30, 35, 40]]
...
# img.reorder_item(bevel_layer, None, len(spread_layers))
